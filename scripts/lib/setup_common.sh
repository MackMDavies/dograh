...
dograh_render_remote_nginx_conf() {
    ...
    {
        echo "# Backend API workers - one uvicorn process per port."
        echo "# Auto-generated by Dograh remote config renderer. Do not edit manually."
        echo "#"
        echo "# Dynamic re-resolution: the /api/v1/ location uses a variable proxy_pass"
        echo "# (proxy_pass http://\$dograh_api_target) plus the resolver below, so nginx"
        echo "# re-resolves the api container IP at request time. A static upstream{} would"
        echo "# resolve it only once at startup, so recreating the api container (new Docker"
        echo "# IP) would strand nginx on a dead address -> 502. split_clients round-robins"
        echo "# the worker ports; keying on \$msec spreads load per request."
        echo "resolver 127.0.0.11 valid=10s ipv6=off;"
        echo ""
        echo "split_clients \"\${remote_addr}\${remote_port}\${msec}\" \$dograh_api_port {"
        # Even split across the worker ports: each of the first N-1 ports gets an
        # explicit percentage bucket, the last is the "*" catch-all.
        pct=$(( 100 / FASTAPI_WORKERS ))
        for ((i=0; i<FASTAPI_WORKERS - 1; i++)); do
            printf '    %d%%%*s%d;\n' "$pct" 5 "" "$((8000 + i))"
        done
        printf '    *%*s%d;\n' 6 "" "$((8000 + FASTAPI_WORKERS - 1))"
        echo "}"
    } > "$tmp_upstream"
...
dograh_preflight_remote_init_render() {
    ...
    # Count the worker ports in the split_clients block: lines like "50%   8000;"
    # or "*     8001;". Must equal FASTAPI_WORKERS.
    nginx_workers=$(awk '/^[[:space:]]*(\*|[0-9]+%)[[:space:]]+[0-9]+;[[:space:]]*$/ { count += 1 } END { print count + 0 }' "$nginx_conf")
    [[ "$nginx_workers" -eq "$FASTAPI_WORKERS" ]] || dograh_fail "FASTAPI_WORKERS=$FASTAPI_WORKERS but nginx.conf has $nginx_workers worker-port entries"
